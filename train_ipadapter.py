                ...
                if accelerator.sync_gradients:
                    ...
                    if cfg.training.checkpoint_interval_type == "steps" and global_step % cfg.training.validation_interval == 0:
                        ...
                        if accelerator.is_main_process:
                            ...
                            try:
                                # 准备验证用的 IP-Adapter 参考图像
                                # 使用 conditioning image 作为所有 6 面的参考
                                val_ip_adapter_images = None
                                if use_ip_adapter:
                                    # 将 conditioning tensor 转为 PIL 用于 IP-Adapter
                                    # 反归一化: (x * 0.5 + 0.5) 将 [-1, 1] 转为 [0, 1]
                                    val_cond_np = (val_conditioning_image.squeeze(0).cpu().permute(1, 2, 0).numpy() * 0.5 + 0.5)
                                    val_cond_np = (val_cond_np * 255).clip(0, 255).astype(np.uint8)
                                    val_cond_pil = Image.fromarray(val_cond_np)
                                    val_ip_adapter_images = [val_cond_pil] * 6  # 6 面使用相同参考
                                ...
                                with torch.no_grad(), torch.amp.autocast('cuda', dtype=weight_dtype):
                                    pipeline_output = pipe(
                                        ...
                                        ip_adapter_image=[val_ip_adapter_images] if val_ip_adapter_images else None,
                            ...
                            # Set model back to train mode and restore dtype
                            # 重要：验证后需要恢复 UNet 的 dtype，否则训练会出现 FP16 梯度错误
