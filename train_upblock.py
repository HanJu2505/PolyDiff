"""
SD1.5 Last Up-Block Training Script

This script uses a targeted fine-tuning strategy:
- Uses original SD1.5 UNet (4-channel input, no position encoding or mask)
- Trains the last up_block (up_blocks[3]) + conv_out layer
- All other layers frozen
- Generates 6 faces independently with IP-Adapter conditioning

Usage:
    python train_upblock.py --config upblock_only
"""
...
def main(cfg: DictConfig):
    ...
    if accelerator.is_main_process:
        ...
        print(f"[INFO] Training mode: up_blocks[3] + conv_out (4-channel input)")
    ...
    # ---------------------- Freeze All, Unfreeze Last Up-Block + conv_out ----------------------
    # 策略：训练最后一个 up_block (up_blocks[3]) 和 conv_out
    ...
    # Unfreeze the last up_block (up_blocks[3])
    for param in pipe.unet.up_blocks[3].parameters():
        param.requires_grad = True
    
    # Also unfreeze conv_out
    for param in pipe.unet.conv_out.parameters():
        param.requires_grad = True
    
    trainable_params = sum(p.numel() for p in pipe.unet.parameters() if p.requires_grad)
    if accelerator.is_main_process:
        print(f"[INFO] Trainable parameters (up_blocks[3] + conv_out): {trainable_params:,}")
